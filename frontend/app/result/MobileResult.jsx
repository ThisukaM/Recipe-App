...
const MobileResult = ({title, cookingTime, ingredients, tags}) => {
    const displayedTags = tags.slice(0, 3);
    return (
        <div className="w-full">
            <div className="w-full flex justify-center items-center flex-col">
                <div className="flex flex-row justify-between items-center w-[90%]">
                    <div className="text-bold text-md">{title}</div>
                    <div className="text-sm text-default-400">{cookingTime}</div>
                </div>
                ...
                <div className="flex flex-row justify-between gap-3 items-center w-[90%]">
                    <p className="text-sm text-default-600">
                        {Array.isArray(ingredients) ? ingredients.join(", ") : "N/A"}
                    </p>
